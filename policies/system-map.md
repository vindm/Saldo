...
| `_pipeline.py` + `config/pipelines/monthly_close.yaml` | the declared 6-stage monthly cycle; `stage_index_of(task_type)` maps a type to its stage. |
...
| `_periods.py` | the **Periods** view — each reporting period × 6 stages' progress across clients; stage chips jump to the Plan. |
...
The declared monthly pipeline lives in `config/pipelines/monthly_close.yaml` (`_pipeline.py`): collect source docs → post to 1C → month close → month audit → calc+notice+payment order (incl. `ausn_monthly`) → sign/pay. The **Periods** view renders progress per period × stage.
